...
private:
	...
	struct Bullet
	{
		int x;			//ÃÑ¾Ë xÁÂÇ¥
		int y;			//ÃÑ¾Ë yÁÂÇ¥
		int speed;		//ÃÑ¾Ë ½ºÇÇµå
		int era;		//ÃÑ¾Ë »èÁ¦ ¿©ºÎ(À©µµ¿ì Å©±â¿¡¼­ ¹þ¾î³ª°Å³ª,Ãæµ¹)
		int mass;
	};

	void missileLaunch(float power,float angle);
	void CharacterJump(float);
	void DrawMap(); //¸Ê±×¸®±â
	bool keyrender(); //Å°È®ÀÎ
	void DrawEnemy();
	void Attack();
	void CreateBullet();
	void DrawBullet(Bullet *tmp);
	void BulletCollision(Bullet *tmp);
...
private:
	...
	ID2D1BitmapBrush *m_pNightSkyBitmapBrush;
	...
	ID2D1BitmapBrush *m_pBossBitmapBrush;
	ID2D1BitmapBrush *m_pMissileBitmapBrush;
	ID2D1BitmapBrush *m_pBulletBitmapBrush;
	ID2D1BitmapBrush *m_pExplosionBitmapBrush;
	ID2D1BitmapBrush *m_pGameOverBitmapBrush;
	ID2D1BitmapBrush *m_pGameClearBitmapBrush;
	ID2D1BitmapBrush *m_pHeartBitmapBrush;

	//bitmap
	IWICImagingFactory *m_pWICFactory;
	ID2D1Bitmap *m_pCharacterBitmap1;
	ID2D1Bitmap *m_pCharacterBitmap2;
	ID2D1Bitmap *m_pBossBitmap;
	ID2D1Bitmap *m_pSkyBitmap;
	ID2D1Bitmap *m_pNightSkyBitmap;
	ID2D1Bitmap *m_pMissileBitmap;
	ID2D1Bitmap *m_pBulletBitmap;
	ID2D1Bitmap *m_pGroundBitmap;
	ID2D1Bitmap *m_pExplosionBitmap;
	ID2D1Bitmap *m_pGameOverBitmap;
	ID2D1Bitmap *m_pGameClearBitmap;
	ID2D1Bitmap *m_pHeartBitmap;
