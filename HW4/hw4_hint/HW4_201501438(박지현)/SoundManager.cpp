// Uses DirectSound to play back sounds
...
bool CSoundManager::add(LPTSTR filename, int *id)
{
	// Create a sound file
	...
	if (FAILED(hr = Create(&newSound, const_cast<LPTSTR>(filename), 0, GUID_NULL)))
		return FALSE;
...
void CSoundManager::stop(int id)
{
	if (id < 0 || id >= (int)m_soundVector.size())
		return;

	if (m_soundVector[id]->IsSoundPlaying()) {
		m_soundVector[id]->Stop();
	}
}

BOOL CSoundManager::SetVolume(int id, LONG lVolume) {
	//lpDSBuffer->SetVolume(DSBVOLUME_MIN);
	if (id < 0 || id >= (int)m_soundVector.size())
		return FALSE;

	if (FAILED(m_soundVector[id]->GetBuffer(id)->SetVolume(DSVOLUME_TO_DB(lVolume))))
		return FALSE;
...
// Desc: Constructs the class
//-----------------------------------------------------------------------------
CSound::CSound(IDirectSoundBuffer** apDSBuffer, DWORD dwDSBufferSize,
	DWORD dwNumBuffers, CWaveFile* pWaveFile, DWORD dwCreationFlags)
{
	...
	m_apDSBuffer = new IDirectSoundBuffer*[dwNumBuffers];
...
//-----------------------------------------------------------------------------
HRESULT CSound::FillBufferWithSound(IDirectSoundBuffer* pDSB, BOOL bRepeatWavIfBufferLarger)
{
	HRESULT hr;
...
//-----------------------------------------------------------------------------
HRESULT CSound::RestoreBuffer(IDirectSoundBuffer* pDSB, BOOL* pbWasRestored)
{
	HRESULT hr;
...
// Desc:
//-----------------------------------------------------------------------------
IDirectSoundBuffer* CSound::GetBuffer(DWORD dwIndex)
{
	if (m_apDSBuffer == NULL)
		return NULL;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CSound::Get3DBufferInterface(DWORD dwIndex, IDirectSound3DBuffer** ppDS3DBuffer)
{
	if (m_apDSBuffer == NULL)
		return CO_E_NOTINITIALIZED;
	if (dwIndex >= m_dwNumBuffers)
...
//-----------------------------------------------------------------------------
CStreamingSound::CStreamingSound(IDirectSoundBuffer* pDSBuffer, DWORD dwDSBufferSize,
