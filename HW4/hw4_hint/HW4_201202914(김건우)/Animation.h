...
public:
  ...
  void SetDuration(T duration) { m_Duration = max(0, duration); }

  T GetDuration() { return m_Duration; }
...
protected:
  virtual T ComputeValue(T time) {
    return (T)(this->m_Start +
      ((this->m_End - this->m_Start) * (time / this->m_Duration)));
  }
};
...
template <typename T>
class AnimationAccel : public Animation<T> {
public:
  AnimationAccel(T start = 0, T end = 0, T duration = 0)
    : Animation<T>(start, end, duration) { }
...
protected:
  virtual T ComputeValue(T time) {
    return
      this->m_Start +
      (this->m_End - this->m_Start) * (time / this->m_Duration) * (time / this->m_Duration);
  }
};
...
protected:
  T ComputeValue(T time) {
    return (T)(this->m_Start + (this->m_End - this->m_Start) *
      pow(2, 10 * (time / this->m_Duration - 1)));
  }
};
